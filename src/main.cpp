...
// =====================================================
// CONFIRMED UPPER-ARC PRIMITIVE
// Tested on the actual ST7789 240x240 display.
// 180 deg = left, 90 deg = top, 0 deg = right.
// =====================================================
void drawConfirmedUpperArc(int cx, int cy, int radius, int thickness,
                           float percent, uint16_t activeColor,
                           uint16_t trackColor) {
  ...
  // Full grey upper semicircle.
  drawPart(0.0f, 180.0f, trackColor);

  // Active value grows from LEFT -> TOP -> RIGHT.
  float endAngle = 180.0f - (percent / 100.0f) * 180.0f;
  drawPart(endAngle, 180.0f, activeColor);
}
...
void drawGauge(
  ...
  if (maxValue > minValue) {
    pct = ((value - minValue) / (maxValue - minValue)) * 100.0f;
  }
  pct = constrain(pct, 0.0f, 100.0f);

  // Page-1 gauges: same geometry proven by the standalone LCD test,
  // only scaled down to fit three gauges across 240 px.
  int thickness = (radius >= 30) ? 6 : 5;
  drawConfirmedUpperArc(cx, cy + 8, radius, thickness,
                        pct, gaugeColor, COL_BORDER);

  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TFT_WHITE, COL_BG);
  tft.setTextSize(1);
  tft.drawString(String(value, decimals), cx, cy - 5, 2);

  tft.setTextColor(COL_DIM, COL_BG);
  tft.drawString(unit, cx, cy + 12, 1);
...
void drawBatteryPage() {
  ...
  // Confirmed upper semicircle geometry from the standalone hardware test.
  // Smaller than the test version so the complete Page-2 UI fits 240x240.
  const int gaugeCX = 120;
  const int gaugeCY = 105;
  const int gaugeRadius = 48;
  const int gaugeThickness = 8;

  drawConfirmedUpperArc(gaugeCX, gaugeCY,
                        gaugeRadius, gaugeThickness,
                        batteryPct, COL_GREEN, COL_BORDER);

  // Values inside the arch
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(COL_TEXT, COL_BG);
  tft.setTextFont(4);
  tft.drawString(String(batteryPct), 110, 76);

  tft.setTextFont(2);
  tft.drawString("%", 145, 78);
  tft.drawString(String(batteryVolt, 1) + " V", 120, 99);
  ...
  tft.setTextFont(1);
  tft.setTextColor(COL_DIM, COL_BG);
  tft.drawString("DISCHARGE CURRENT", 10, 154);

  tft.setTextDatum(TR_DATUM);
  tft.setTextColor(COL_TEXT, COL_BG);
  tft.drawString(String(batteryCurrent, 1) + " A", 230, 154);
  ...
  // Footer
  tft.drawFastHLine(8, 187, 224, COL_BORDER);
  drawBattery(12, 198, batteryPct);

  tft.setTextDatum(TL_DATUM);
  tft.setTextFont(1);
  tft.setTextColor(COL_DIM, COL_BG);
  tft.drawString("STATUS", 68, 204);

  tft.setTextColor(COL_GREEN, COL_BG);
  tft.drawString("NORMAL", 108, 204);

  tft.fillCircle(218, 208, 7, COL_GREEN);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TFT_BLACK, COL_GREEN);
  tft.drawString("OK", 218, 208, 1);
